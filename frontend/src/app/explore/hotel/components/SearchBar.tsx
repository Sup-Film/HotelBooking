const SearchBar = ({ variant }: { variant: "desktop" | "mobile" }) => {
  if (variant === "mobile") {
    return (
      <div className="flex w-full flex-col items-center gap-4 py-6">
        ...
        <button
          type="button"
          className="w-full border border-gray-200 bg-[#f1f3ff] px-4 py-3 text-left text-sm text-gray-700 outline-none"
        >
  ...
  return (
    <div className="flex w-full items-center gap-4 px-8 py-6">
      ...
      <button
        type="button"
        className="w-[260px] border border-gray-200 bg-[#f1f3ff] px-4 py-3 text-left text-sm text-gray-700 outline-none"
      >
        2 adult ,0 children - 1 room
      </button>
      <button
