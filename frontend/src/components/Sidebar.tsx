...
const menuItems = [
  ...
  { name: "Explore", path: "/explore", icon: FaMagnifyingGlass },
  { name: "Trips", path: "/trips", icon: FaHeart },
              ...
              <div key={item.name} className="flex flex-col items-center gap-2">
                ...
                >
                  <item.icon
                    size={24}
                    className="mx-auto"
                    color={isActive ? "#2d36d9" : "white"}
                  />
                  <span className={isActive ? "mt-1" : ""}>{item.name}</span>
                </div>
              </div>
            );
          })}
